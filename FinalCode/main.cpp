...
int find_index(double x_position, double y_position, vector<double> x_points, vector<double> y_points)
{
...
int main ()
{
  ...
  **/
  ...
  //pid_steer.Init(0.35, 0.00025, 0.10, 1.2, -1.2);
 //pid_steer.Init(0.2, 0.0011, 0.8, 1.2, -1.2); // Bad

  // pid_steer.Init(0.3, 0.0011, 0.7, 1.2, -1.2); //Good

  // pid_steer.Init(0.25, 0.0011, 0.5, 1.2, -1.2); // very good

  // pid_steer.Init(0.25, 0.0009, 0.5, 1.2, -1.2); // excellent First run

 // pid_steer.Init(0.2, 0.0009, 0.4, 1.2, -1.2); // excellemnt Second run


  pid_steer.Init(0.2, 0.0009, 0.35, 1.2, -1.2); // excellent fourth time
...
//           // Update the delta time with the previous command
          ...
          **/
          int index =  find_index(x_position, y_position, x_points, y_points);

          //error_steer = angle_between_points(x_position, y_position, x_points[index], y_points[index]) - yaw;
      	  error_steer = yaw - angle_between_points(x_position, y_position, x_points[index], y_points[index]) + y_position - y_points[index] ;
          /**
          * TODO (step 3): uncomment these lines
          **/
